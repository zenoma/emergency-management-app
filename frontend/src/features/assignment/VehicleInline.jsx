...
export default function VehicleInline({ onSelect, organizations = [], externalSelectedId = null }) {
  const { t, i18n } = useTranslation();
  const token = useSelector(selectToken);
  const locale = i18n?.language || 'es';

  const { data: vehicles = [] } = useGetActiveVehiclesQuery({ token, locale });
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, mt: 1 }}>
      <Typography variant="subtitle1" sx={{ mb: 1 }}>{t('vehicle-selection', 'Vehicles')}</Typography>
      <Grid container spacing={2} sx={{ mb: 1 }}>
        <Grid item xs={12} md={6}>
          <TextField fullWidth label={t('filter', 'Filter')} value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('filter-placeholder', 'Search by plate, type or id')} />
        </Grid>
        ...
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>{t('plate', 'Plate')}</TableCell>
            ...
            {selected ? (
              <TableRow>
                <TableCell colSpan={5}>
                  <Paper variant="outlined" sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <Typography sx={{ fontWeight: 700 }}>{selected.vehiclePlate || selected.plate || selected.code} (#{selected.id})</Typography>
              ...
              filtered.map((r) => (
                <TableRow key={r.id} hover>
                  ...
                  <TableCell>
                    <Typography sx={{ fontWeight: 600 }}>{r.vehiclePlate || r.plate || r.code}</Typography>
