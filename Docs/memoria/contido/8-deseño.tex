...
En la figura \ref{fig:swagger-collection} se muestra la documentación generada mediante Swagger, que refleja los bloques de gestión de organización, de emergencias y de recomendación. El resto de bloques también se ha documentado, pero se han omitido en la imagen para no saturar la vista. Aquí se pueden observar las ventajas de este enfoque, ya que la documentación se genera de forma automática a partir del contrato definido en OpenAPI, lo que ayuda a mantenerla sincronizada con la implementación real de los endpoints. Además, esta vista permite comprobar de forma centralizada los recursos disponibles, los métodos HTTP asociados y la estructura general de la interfaz pública del backend. También se observan en la documentación los endpoints que requieren autenticación y los que son accesibles para usuarios anónimos, lo que refleja la política de seguridad implementada.
...
Como se puede ver en la imagen \ref{fig:c4-container-frontend}, la interfaz se divide por dominios funcionales. \textit{Dashboard} reúne el mapa, la información meteorológica y el panel de avisos, y se mantiene como una de las piezas centrales de la aplicación. \textit{LandingMap} dibuja cuadrantes y emergencias sobre MapLibre, con estilos servidos por MapTiler, e iconos distintos según el tipo de emergencia y recursos desplegados. \textit{Notice} y \textit{MyNoticesList} muestran avisos con sus imágenes y estados, y \textit{CoordinatorNoticesView} añade acciones de aceptación, rechazo y borrado.
