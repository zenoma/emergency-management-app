...
\begin{itemize}
    ...
    \item \textbf{API-first con OpenAPI:} En este proyecto se ha adoptado un diseño \textit{API-first}, definiendo primero el contrato de la API en un fichero YAML con OpenAPI y generando a partir de él parte de los controladores y los DTOs del backend mediante el plugin de Maven de OpenAPI. Gracias a esto se ha generado la documentación mediante Swagger de forma automática, lo que facilita la sincronización entre la implementación y la documentación, y reduce errores de desalineación entre ambos. Este enfoque ha permitido que se generen contratos a los que la aplicación web y la aplicación Android puedan adaptarse de forma consistente, lo que ha sido especialmente útil para integrar nuevas funcionalidades sin necesidad de modificar manualmente los controladores o los contratos de datos.
...
En la figura \ref{fig:c4-container-backend} se aprecia el contenedor backend y su descomposición en controladores, servicios fachada, modelo de dominio, repositorios, seguridad y gestión de históricos. La vista resume la estructura interna sin entrar en componentes más finos. Se ha utilizado el color verde para resaltar los componentes nuevos o significativamente modificados respecto al trabajo previo.

\subsection{Modelo del dominio}

\subsubsection{Diagrama de entidades}

El modelo de dominio se ha ampliado de forma notable respecto al trabajo previo, se ha utilizado el color verde para resaltar las nuevas entidades. La figura \ref{fig:modelo-dominio} resume la estructura persistente actual y permite observar cómo el dominio ha pasado de estar centrado en incendios, recursos y avisos a incorporar de forma explícita emergencias, asignaciones, reglas de recomendación y una gestión de históricos más detallada.
...
Es importante aclarar que se han migrado todos los endpoints existentes a esta nueva forma de trabajo, lo que ha supuesto un esfuerzo de adaptación pero ha permitido mantener la coherencia en el resto del proyecto. Esta migración es agnóstica respecto a la estructura interna del backend, ya que el código generado se integra con las fachadas y servicios de lógica de negocio sin necesidad de modificar su organización. 
...
En la figura \ref{fig:swagger-collection} se muestran la documetación generada mediante Swagger, que refleja los bloques de gestión de organización, de emergencias y de recommendación. El resto de bloques también se ha documentado, pero se han omitido en la imagen para no saturar la vista. Aquí se puede observar las ventajas de este enfoque, ya que la documentación se genera de forma automática a partir del contrato definido en OpenAPI, lo que garantiza que siempre esté sincronizada con la implementación real de los endpoints. Además, esta vista permite comprobar de forma centralizada los recursos disponibles, los métodos HTTP asociados y la estructura general de la interfaz pública del backend. También se observa en la documentación los endpoints que requieren autenticación y los que son accesibles para usuarios anónimos, lo que refleja la política de seguridad implementada. La política de seguridad se ha definido siguiendo el paradigma de seguridad basada en roles, lo que permite controlar el acceso a cada endpoint según el rol del usuario autenticado.


\subsubsection{Gestión de errores e internalización}

La gestión de errores se centraliza en \textit{GlobalControllerExceptionHandler}. Allí se traducen excepciones de instancia, de dominio y de validación a respuestas HTTP adecuadas y mensajes localizados. Este mecanismo viene del trabajo previo y apenas ha necesitado modificaciones estructurales, ya que sigue siendo útil para homogeneizar la respuesta ante errores y para mantener la internacionalización de mensajes.
...
El frontend web ofrece la interfaz principal de gestión y consulta. Respecto a la etapa anterior, no se han producido cambios sustanciales en la arquitectura general, sino una ampliación de vistas y funcionalidades que se ha apoyado en la base ya existente. La aplicación debe seguir permitiendo autenticación, navegación por módulos, visualización de emergencias sobre mapa, gestión de recursos, asignaciones, avisos y reglas de recomendación, además de adaptarse a distintos roles sin romper la experiencia de uso.
...
En la práctica, el trabajo realizado en esta capa se ha centrado en ampliar componentes existentes y en añadir pantallas nuevas, no en cambiar la organización general. Por eso, el esfuerzo principal ha estado en refinar las vistas, integrar las nuevas funcionalidades y mantener el aspecto funcional con el frontend ya desarrollado anteriormente.
...
También se ha migrado a \textit{MapLibre}, una librería open source que permite prescindir de planes gratuitos o limitados. A mayores se usan librerías como \textit{Material UI} y \textit{dayjs}. Estas integraciones ya estaban presentes en la etapa anterior y se han mantenido porque encajan bien con la evolución del sistema y permiten incorporar nuevas funcionalidades sin rehacer la base del frontend.
...
La arquitectura de la aplicación Android se organiza en torno a una actividad principal que actúa como contenedor de la experiencia completa. Desde esa actividad se resuelve la navegación entre pantallas, que se construyen principalmente con Jetpack Compose, aunque en algunos casos se mantienen fragments o activities para flujos específicos. 
...
La gestión del equipo propio se concentra en \textit{MyTeamCompose} y \textit{MyAssignmentsScreen}. La primera pantalla consulta el equipo del usuario y sus miembros; la segunda lista las asignaciones pendientes del recurso del usuario y permite aceptarlas desde el propio dispositivo. Este flujo es una de las piezas más relevantes del cliente móvil, porque conecta la notificación recibida con una acción inmediata.
...
En esta capa también destaca el comportamiento del menú lateral: si el usuario no ha iniciado sesión se muestra un acceso directo al login, mientras que si existe sesión activa se muestran el avatar, el nombre y los bloques de navegación disponibles según el rol. Además, el propio menú calcula el número de asignaciones pendientes y lo muestra como indicador, lo que ofrece una visión rápida del trabajo pendiente sin entrar en cada pantalla.
