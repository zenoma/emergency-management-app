...
fun MapScreen(modifier: Modifier = Modifier) {
    ...
    // ensure MapLibre initialized
    try { MapLibre.getInstance(context) } catch (_: Exception) {}

    // remember the MapView instance so we can forward lifecycle events
    val mapView = remember { MapView(context) }

    // create the MapView once
    DisposableEffect(mapView) {
        ...
        onDispose {
            ...
            if (lifecycleOwner != null && observer != null) lifecycleOwner.lifecycle.removeObserver(observer)
        }
    }
                    ...
                    Thread {
                        ...
                        (ctx as? android.app.Activity)?.runOnUiThread {
                            try {
                                ...
                                try {
                                    if (!emergenciesJson.isNullOrBlank()) addOrUpdateEmergencyMarkers(map, emergenciesJson, ctx)
                                } catch (e: Exception) {
                                    android.util.Log.w("MapScreen", "Failed to add emergency markers", e)
                                }
                            } catch (e: Exception) {
                                android.util.Log.w("MapScreen", "Failed to add/update quadrants source/layer", e)
                            }
                        }
                    }.start()
...
private fun chooseEmergencyIconName(typeKey: String?): String {
    ...
    val n = try {
        java.text.Normalizer.normalize(lowered, java.text.Normalizer.Form.NFD).replace("\\p{InCombiningDiacriticalMarks}+".toRegex(), "")
    } catch (e: Exception) {
        lowered
    }
    return when {
        ...
        n.contains("torment") || n.contains("temporal") || n.contains("storm") || n.contains("meteor") || n.contains("evento") -> "emergency-storm"
...
private fun bitmapFromDrawable(context: Context, resId: Int, sizePx: Int): Bitmap {
    val drawable = androidx.core.content.ContextCompat.getDrawable(context, resId) ?: return Bitmap.createBitmap(1, 1, Bitmap.Config.ARGB_8888)
...
private fun ensureEmergencyImagesAndLayer(style: org.maplibre.android.maps.Style, context: Context) {
    try {
        ...
        val mapping = mapOf(
            ...
            "emergency-medical" to arrayOf("emergencia_medica", "emergencia-medica", "emergencia_medica"),
            "emergency-industrial" to arrayOf("riesgo_industrial", "riesgo-industrial", "riesgo_industrial"),
            "emergency-montana" to arrayOf("montana"),
            "emergency-chemical" to arrayOf("quimical_hazard", "quimical hazard", "quimical_hazard"),
        ...
        for ((key, candidates) in mapping) {
            try {
                ...
                if (id != 0) {
                    ...
                    val redBmp = try { tintBitmapToColor(bmp, Color.RED) } catch (t: Throwable) { bmp }
                    style.addImage(key, redBmp)
                } else {
                    android.util.Log.w("MapScreen", "No drawable found for map icon '$key' (candidates=${candidates.joinToString()})")
                }
            } catch (ex: Exception) {
...
private fun addOrUpdateEmergencyMarkers(map: org.maplibre.android.maps.MapLibreMap, emergenciesJson: String, ctx: Context) {
    try {
        val style = map.style
        if (style != null) addOrUpdateEmergencySource(style, emergenciesJson, ctx) else map.getStyle { s -> addOrUpdateEmergencySource(s, emergenciesJson, ctx) }
    } catch (e: Exception) {
        android.util.Log.w("MapScreen", "Failed to add/update emergency markers", e)
    }
}

private fun addOrUpdateEmergencySource(style: org.maplibre.android.maps.Style, emergenciesRaw: String, ctx: Context) {
    try {
        ...
        for (i in 0 until arr.length()) {
            try {
                ...
                if ((e.has("resolvedAt") && !e.isNull("resolvedAt")) || e.optString("status", "").equals("resolved", ignoreCase = true)) continue
                val loc = when {
                ...
                val lon = if (loc.has("lon")) loc.optDouble("lon", Double.NaN) else if (loc.has("x")) loc.optDouble("x", Double.NaN) else Double.NaN
                val lat = if (loc.has("lat")) loc.optDouble("lat", Double.NaN) else if (loc.has("y")) loc.optDouble("y", Double.NaN) else Double.NaN
        ...
        if (existing != null) {
            try { existing.setGeoJson(fc.toString()) } catch (e: Exception) { android.util.Log.w("MapScreen", "Failed to set emergencies geojson on existing source", e) }
        } else style.addSource(GeoJsonSource("emergencies", fc.toString()))
    } catch (e: Exception) { android.util.Log.w("MapScreen", "Failed to build emergency source", e) }
}
    ...
    return try {
        ...
        if (trimmed.startsWith("[")) {
            ...
            for (i in 0 until arr.length()) {
                ...
                val geom = org.json.JSONObject(); geom.put("type", "MultiPolygon"); geom.put("coordinates", multipoly)
                val props = org.json.JSONObject(); props.put("id", q.optInt("id")); props.put("nombre", q.optString("nombre"))
                val feat = org.json.JSONObject(); feat.put("type", "Feature"); feat.put("properties", props); feat.put("geometry", geom)
                features.put(feat)
            }
            val fc = org.json.JSONObject(); fc.put("type", "FeatureCollection"); fc.put("features", features); fc.toString()
        } else {
...
private fun addOrUpdateQuadrantsSource(style: org.maplibre.android.maps.Style, jsonToUse: String) {
    ...
    if (existing != null) {
        try { existing.setGeoJson(jsonToUse); android.util.Log.d("MapScreen", "Updated existing quadrants source") } catch (e: Exception) { android.util.Log.w("MapScreen", "Failed to set geojson on existing source", e) }
    } else {
        style.addSource(GeoJsonSource("quadrants", jsonToUse))
        style.addLayer(FillLayer("quadrants-layer", "quadrants").withProperties(fillColor("#D62B2B"), fillOpacity(0.5f)))
        try { style.addLayer(LineLayer("quadrants-border", "quadrants").withProperties(lineColor("#8B0000"), lineWidth(2f))) } catch (e: Exception) { android.util.Log.w("MapScreen", "Could not add border layer", e) }
